...
rhythm.make_multi_gliss(
    ...
    preprocessor=trinton.fuse_quarters_preprocessor((3, 2, 1)),
    finger_percussion="first",
    finger_percussion_padding=4,
    finger_percussion_voice_index=0,
    extra_voice="sub 4",
...
for voice_name, leaf_ranges in zip(
    [
        r"cello 2 voice gliss 1 sub 4",
        r"cello 2 voice gliss 2 sub 4",
        r"cello 2 voice gliss 3 sub 4",
        r"cello 2 voice gliss 4 sub 4",
    ],
    [
        [(8, 16), (16, 22)],
        [(7, 14), (14, 19)],
    ...
    trinton.make_music(
        ...
        trinton.octavation(
            ...
            selector=trinton.select_leaves_by_index(
                list(range(leaf_ranges[0][0], leaf_ranges[0][-1]))
            ),
        ),
        trinton.octavation(
            octave=-2,
            selector=trinton.select_leaves_by_index(
                list(range(leaf_ranges[1][0], leaf_ranges[1][-1]))
            ),
        ),
...
trinton.make_music(
    ...
    trinton.pitch_with_selector_command(
        pitch_list=["g,"], selector=trinton.select_leaves_by_index([-1], pitched=True)
    ),
    voice=score[r"cello 2 voice gliss 3 sub 4"],
)
...
trinton.make_music(
    ...
    trinton.linear_attachment_command(
        ...
        selector=trinton.select_leaves_by_index([13, 14], pitched=True, grace=False),
    ),
    trinton.hooked_spanner_command(
        string=r"""\markup { "finger perc." }""",
    ...
    trinton.hooked_spanner_command(
        string=r"""\markup { "arco ( bowing close to LH as possible )" }""",
    ...
    voice=score["cello 2 voice gliss 1 sub 4"],
)
...
trinton.make_music(
    ...
    trinton.pitch_with_selector_command(
        ...
        selector=trinton.select_leaves_by_index([0, 1, 2, 3, 4, 5, 6, 7]),
    ),
    trinton.pitch_with_selector_command(
        pitch_list=[
            ["c'", "a", "f", "d"],
        ...
        selector=trinton.select_leaves_by_index([8, 9, 10, 11, 12, 13, 14]),
    ),
    trinton.attachment_command(
    ...
    trinton.invisible_accidentals_command(
        selector=trinton.select_leaves_by_index([1, 2, 3, 4, 5, 6, 7]),
    ),
    library.change_staff_type(
    ...
    library.pressure_trills(
        ...
        # bound_details=(1.5, 1.5),
        right_padding=-4.5,
        selector=trinton.select_leaves_by_index([11, 12], pitched=True, grace=False),
    ),
    trinton.spanner_command(
        strings=["mst.", "dp."],
        selector=trinton.select_logical_ties_by_index(
            [0, 7, 7, -1], pitched=True, first=True, grace=False
        ),
        style="solid-line-with-arrow",
        padding=8,
        right_padding=0,
...
trinton.make_music(
    ...
    trinton.attachment_command(
        attachments=[
            abjad.LilyPondLiteral(
                r"\once \override DynamicLineSpanner.staff-padding = #13", site="before"
            ),
            abjad.Dynamic('"ff"'),
        ],
        selector=trinton.select_leaves_by_index([-1], pitched=True, grace=False),
...
trinton.make_music(
    ...
    trinton.linear_attachment_command(
        attachments=[abjad.Dynamic("p"), abjad.Dynamic('"ff"')],
        selector=trinton.select_leaves_by_index([0, -1], pitched=True, grace=False),
    ),
    voice=score["cello 2 voice"],
)
...
trinton.make_music(
    ...
    evans.RhythmHandler(
        rhythm.weighted_talea(
            start_index=6, prolations=[4, 6, 7], elaboration_level=0, retrograde=False
        )
    ),
    trinton.rewrite_meter_command(boundary_depth=-1),
    voice=score["cello 2 voice"],
...
trinton.make_music(
    ...
    evans.RhythmHandler(evans.talea([7, 2, 1, 2], 16)),
    ...
    trinton.hooked_spanner_command(
        ...
        tweaks=[
            r"""- \tweak font-name "Bodoni72 Book Italic" """,
            r"""- \tweak font-size 2""",
            # r"""- \tweak bound-details.left.Y #6.5""",
...
trinton.make_music(
    lambda _: trinton.select_target(_, (20, 24)),
    trinton.duration_line(
        selector=trinton.logical_ties(
            exclude=[0, 1, -1, -2, -3], pitched=True, grace=False
        ),
    ),
    trinton.attachment_command(
        attachments=[
        ...
        selector=trinton.pleaves(exclude=[-1, -2, -3, -4], grace=False),
    ),
    trinton.noteheads_only(selector=trinton.pleaves(grace=True)),
    trinton.hooked_spanner_command(
        ...
        tweaks=[
            r"""- \tweak font-name "Bodoni72 Book Italic" """,
            r"""- \tweak font-size 2""",
            r"""- \tweak bound-details.left.Y #8""",
            r"""- \tweak bound-details.right.Y #1.5""",
        ],
    ),
    voice=score["cello 2 voice"],
)
...
trinton.make_music(
    ...
    evans.RhythmHandler(
        evans.talea([-2, 10, 1, 1, 1, 1, 1, 1, 1, -1, 1, -2, 1, -1, 1, -5, 1, -100], 16)
    ),
    trinton.rewrite_meter_command(boundary_depth=-1),
    evans.PitchHandler([["b", "ds'"]]),
    library.multiple_muting(closed_fundamental=False, forgo_notehead_change=False),
    library.change_staff_type(
        selector=trinton.select_leaves_by_index([0], pitched=True),
        staff_type="reversion",
        auto_reversion=False,
    ...
    trinton.linear_attachment_command(
        attachments=itertools.cycle([abjad.StartBeam(), abjad.StopBeam()]),
        selector=trinton.select_leaves_by_index(
            [0, 1, 3, 14, 15, 16, 18, 19], grace=False
        ),
    ),
    trinton.linear_attachment_command(
    ...
    trinton.spanner_command(
        strings=["", "ord."],
        selector=trinton.select_leaves_by_index([2, 8, 8, -1], pitched=True),
        style="solid-line-with-arrow",
        padding=8,
        right_padding=0,
        direction=None,
    ...
    trinton.attachment_command(
        attachments=[
            abjad.bundle(
                abjad.Markup(r"""\markup { \hspace #-7 "( w/ plectrum )" }"""),
                r"""- \tweak font-name "Bodoni72 Book Italic" """,
                r"""- \tweak font-size 2""",
                r"- \tweak padding 2",
            ),
...
trinton.make_music(
    ...
    evans.RhythmHandler(
        evans.talea(
            [-8, 4, 3, 3, 4, 3, 4, 3, 3, 4, 3, 2, 4, 3, 4, 3, 2, 4, 3, 3, 4, 3, 2], 16
        )
    ),
    library.change_staff_type(
        selector=trinton.select_leaves_by_index([0], pitched=True),
    ...
    trinton.attachment_command(
        attachments=[
            abjad.LilyPondLiteral(
                [
                    r"\once \override Staff.Clef.X-extent = ##f",
                    r"\once \override Staff.Clef.extra-offset = #'(-9 . 0)",
                ],
                site="before",
            )
        ],
...
trinton.make_music(
    lambda _: trinton.select_target(_, (22, 23)),
    evans.RhythmHandler(evans.talea([-8, 4, 6, 8, 11], 16)),
    trinton.rewrite_meter_command(boundary_depth=-1),
    evans.PitchHandler(
        [
            ["f,", "d,"],
    ...
    trinton.attachment_command(
        attachments=[
            abjad.bundle(
                abjad.Markup(r"""\markup { "( w/o plectrum )" }"""),
                r"""- \tweak font-name "Bodoni72 Book Italic" """,
                r"""- \tweak font-size 2""",
            ),
        ],
    ...
    trinton.spanner_command(
        strings=[
            ...
            library.fingering_markup(fingering="i - c"),
            library.fingering_markup(fingering="p - c"),
        ],
        selector=trinton.select_logical_ties_by_index(
            [0, 1, 1, 2, 2, 3], pitched=True, first=True, grace=False
        ),
        style="solid-line-with-arrow",
        padding=10,
        right_padding=0,
    ...
    trinton.hooked_spanner_command(
        string=r"""\markup { "xt." }""",
        selector=trinton.select_leaves_by_index([0, -1], pitched=True),
        padding=14,  # fingering spanner +4
        direction=None,
        right_padding=10,
        full_string=True,
        style="dashed-line-with-hook",
...
for measure in [1, 2, 3, 4, 6, 7, 8, 10, 11, 13, 14, 16, 17, 18, 20, 21, 22, 23]:
    trinton.make_music(
        lambda _: trinton.select_target(_, (measure,)),
        trinton.attachment_command(
...
trinton.make_music(
    ...
    trinton.attachment_command(
        attachments=[
            abjad.LilyPondLiteral(
                r"\once \override Score.NonMusicalPaperColumn.line-break-system-details = #'((alignment-distances . (0 11 34 21)))",
                site="absolute_before",
            ),
            # abjad.bundle(
            #     abjad.Markup(r"\markup { S }"),
...
trinton.make_music(
    ...
    trinton.attachment_command(
        attachments=[
            abjad.LilyPondLiteral(
                r"\once \override Score.NonMusicalPaperColumn.line-break-system-details = #'((alignment-distances . (4 31)))",
                site="absolute_before",
            ),
            # abjad.bundle(
            #     abjad.Markup(r"\markup { S }"),
...
trinton.make_music(
    ...
    trinton.attachment_command(
        attachments=[
            abjad.LilyPondLiteral(
                r"\once \override Score.NonMusicalPaperColumn.line-break-system-details = #'((alignment-distances . (5 31 16)))",
                site="absolute_before",
            ),
            # abjad.bundle(
            #     abjad.Markup(r"\markup { S }"),
